    ...
    const prepareTransaction = async () => {
      ...
      try {
        ...
        if (this._clientTransaction) {
          ...
          const txnId = await this._clientTransaction.generateTransactionId(init.method || 'GET', urlPath);
          // Always overwrite with a fresh transaction ID bound to this specific URL path
          if (init.headers && typeof init.headers === 'object') {
            (init.headers as Record<string, string>)['x-client-transaction-id'] = txnId;
          }
        }
      } catch {
